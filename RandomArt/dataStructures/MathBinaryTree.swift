...
class MathBinaryTree {
    ...
    // If new math operations are added into either arrays then the code won't change.
    private let binaryOperations: [String] = ["*", "avg"]
    private let uniryOperations: [String] = ["cos", "sin"]//, "floor", "ceiling"]
    ...
    // x and y must be a number from 0 to 1
    ...
    func evaluateTreeHelper(node: Node<String>?, x: Float, y: Float)->Float{
        ...
        switch node.value {
            ...
            case "/":
                ...
                return evaluateTreeHelper(node: node.rightChild , x: x, y: y)
                    / evaluateTreeHelper(node: node.leftChild, x: x, y: y)
            case "sin":
                // TODO: mess around with constant Pi to make it random
            ...
            case "cos":
                return abs(cos(evaluateTreeHelper(node: node.rightChild , x: x, y: y)
                    * (Float.pi)))
            case "floor":
                return abs(evaluateTreeHelper(node: node.rightChild , x: x, y: y) - 1)
            case "ceiling":
                return evaluateTreeHelper(node: node.rightChild , x: x, y: y) + 1
            case "avg":
                return  (evaluateTreeHelper(node: node.rightChild , x: x, y: y)
